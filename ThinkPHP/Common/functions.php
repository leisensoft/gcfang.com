...
/**■■■■■■■■■■■■■■■■■■■■■■
	 * 通过给定时间戳算过去的时间   LJL 2014年8月21日 23:44:27
	 * @param string $time $pattern 默认default mn只显示月日
	 * @access public
	 * @return 
*/
function countPastTime($time,$pattern=0) {
	$time = strtotime($time);
	$timePast=intval((time()-$time)/86400);
	if($timePast){
			if($timePast>10){
				switch($pattern){
					case 1:                                                               
						$timePast=date("m-d",$time);
					break;
					default:$timePast=date("Y-m-d",$time);
					}
			}else{
			$timePast = $timePast.'天前';
			}
	}else{
		$timePast=intval((time()-$time)/3600);
		if($timePast){
			$timePast = $timePast.'小时前';
		}else{
			$timePast=intval((time()-$time)/60).'分钟前';
		}
	}
	return $timePast;
}
