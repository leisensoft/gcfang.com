<?php if (!defined('THINK_PATH')) exit();?><div id="content">
    ...
    url="http://<?php echo ($_SERVER['SERVER_NAME']); ?>/benben/getArticlesInfoList"
...
//■■■■■■■■■■■■■■■■■■■■■■ GIRD	
		...
		function newArticle(){
			...
			url = 'http://<?php echo ($_SERVER['SERVER_NAME']); ?>/benben/updateArticleInfo';
		}
		function editArticle(){
			...
			if (row){
				...
				url = 'http://<?php echo ($_SERVER['SERVER_NAME']); ?>/benben/updateArticleInfo?id='+row.id;
			}
		}
		...
		function destroyArticle(){
			...
			if (row){
				...
				if (r){
					$.post('http://<?php echo ($_SERVER['SERVER_NAME']); ?>/benben/deleteArticleInfo',{id:row.id,state:2},function(result){
						if (result.success){
							$('#articlesInfoTable').datagrid('reload'); // reload the article data
						} else {
					...
					$.post('http://<?php echo ($_SERVER['SERVER_NAME']); ?>/benben/deleteArticleInfo',{id:row.id,state:1},function(result){
						if (result.success){
							$('#articlesInfoTable').datagrid('reload'); // reload the article data
						} else {
							$.messager.show({ // show error message
		...
		$('#articlesInfoTable').datagrid({onDblClickRow: function (rowIndex, rowData) { //■■■■■■■■■■■■■■■■■■■■■■双击表
				  window.open('http://<?php echo ($_SERVER['SERVER_NAME']); ?>/index/news/id/'+rowData.id);
				}       
		}); 
</script>	
<style type="text/css">
